...
#define CODE_POS (dqueue::utils::CodePos(__FILE__, __LINE__, __FUNCTION__))

#define MAKE_EXCEPTION(msg) dqueue::utils::Exception::create_and_log(CODE_POS, msg)
// macros, because need CODE_POS

#ifdef DEBUG
#define THROW_EXCEPTION(...)                                                             \
  dqueue::utils::Exception::create_and_log(CODE_POS, __VA_ARGS__);                       \
  std::exit(1);
#else
#define THROW_EXCEPTION(...)                                                             \
  throw dqueue::utils::Exception::create_and_log(CODE_POS, __VA_ARGS__);
#endif
