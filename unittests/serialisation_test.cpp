...
TEST_CASE("serialisation.scheme") {
	...
	serialisation::Scheme<int>::Writer<iterator_t> w(it, 1);
	auto sz = std::distance(buffer.begin(), it);
	EXPECT_EQ(sz, sizeof(int));

	it = buffer.begin();
	serialisation::Scheme<int, int>::Writer<iterator_t> i2(it, 1, 2);
	...
	serialisation::Scheme<int, int>::Reader::read(it, unpacked1, unpacked2);
	EXPECT_EQ(unpacked1, 1);
	EXPECT_EQ(unpacked2, 2);
	
	it = buffer.begin();
	std::string str = "hello world";
	serialisation::Scheme<int, std::string>::Writer<iterator_t> s1(it, 11, std::move(str));
	sz = std::distance(buffer.begin(), it);
	EXPECT_EQ(size_t(sz), size_t(sizeof(int) + sizeof(uint32_t) + str.size()));

	it = buffer.begin();
	std::string unpackedS;
	serialisation::Scheme<int, std::string>::Reader::read(it, unpacked1, unpackedS);
